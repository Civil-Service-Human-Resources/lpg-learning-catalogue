            ...
            steps {
                ...
                script {
                    docker.withRegistry("${env.DOCKER_REGISTRY_URL}", 'docker_registry_credentials') {
                        def customImage = docker.build("lpg-learning-catalogue:${env.BUILD_ID}")
                        customImage.push()
                    }
                }
                stash 'workspace'
            ...
            steps {
                ...
                withCredentials([
                    ...
                    dir("lpg-terraform-paas/environments/master") {
                        ...
                        sh "terraform plan -target=module.lpg-learning-catalogue -var 'learning_catalogue_docker_tag=${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}'"
                        sh "terraform apply -target=module.lpg-learning-catalogue -var 'learning_catalogue_docker_tag=${env.BUILD_ID}' -var 'docker_registry_server_username=${acr_username}' -var 'docker_registry_server_password=${acr_password}' -auto-approve"
                    }
                }
            }
        }
